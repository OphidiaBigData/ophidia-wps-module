...
import subprocess
import StringIO
from PyOphidia import cube, ophsubmit as _ophsubmit
...
class oph_subset(WPSProcess):
	...
	def execute(self):
		...
		logging.debug("Set input cube")
		inputcube = cube.Cube(pid=self.cube.getValue())

		logging.debug("Execute the job")
		try:
			inputcube.subset(subset_dims=self.subset_dims.getValue(), subset_filter=self.subset_filter.getValue(), container=self.container.getValue(), exec_mode=self.exec_mode.getValue(), grid=self.grid.getValue(), ncores=self.ncores.getValue(), schedule=self.schedule.getValue(), description=self.description.getValue(), display=False)
		except Exception as e:
			buffer = None
			jobid = None
			return_value = cube.Cube.client.last_return_value
			error = cube.Cube.client.last_error
		else:
			buffer = cube.Cube.client.last_response
			jobid = cube.Cube.client.last_jobid
			return_value = 0
			error = None
