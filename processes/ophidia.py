...
class oph_cubeschema(WPSProcess):
    ...
    def __init__(self):
        ...
        self.dim = self.addLiteralInput(
            identifier="dim",
            title="Dimension name",
            abstract="Name of dimension to show. Only valid with level bigger than 0. Multiple-value field: list of dimensions separated by '|' can be provided. If not specified, all dimensions are shown",
            minOccurs=0,
            maxOccurs=1,
            default="all",
            type=type(''))
...
class oph_metadata(WPSProcess):
    ...
    def __init__(self):
        ...
        self.variable = self.addLiteralInput(
            identifier="variable",
            title="Variable",
            abstract="Name of the variable to which we can associate a new metadata key. THe value is considered only in insert mode and its defaylt value 'global' can be used to insert a global metadata",
            minOccurs=0,
            maxOccurs=1,
            default="global",
            type=type(''))
...
class oph_deletecontainer(WPSProcess):
    ...
    def __init__(self):
        ...
        self.userid = self.addLiteralInput(
            identifier="userid",
            title="Username",
            abstract="User removes for Ophidia system an emplty container with related dimensions and grids. The container can be deleted logically or physically",
            type=type(''))
...
class oph_instances(WPSProcess):
    ...
    def __init__(self):
        ...
        self.host_partition = self.addLiteralInput(
            identifier="host_partition",
            title="Host partition",
            abstract="Optional filter on host partition name, Used only with level 3. If no partition is specified, then the list of all partitions is shown",
            minOccurs=0,
            maxOccurs=1,
            default="all",
            type=type(''))
